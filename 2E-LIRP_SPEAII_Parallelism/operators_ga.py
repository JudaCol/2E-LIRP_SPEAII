...
# funcion que corre el algoritmo genetico en totalidad
def run_ga(p_externa, n_clientes, n_centroslocales, n_centrosregionales, n_periodos, n_productos, n_vehiculos_s, n_vehiculos_p, capacidad_cr, capacidad_cl, capacidad_vehiculos_p, capacidad_vehiculos_s, demanda_clientes, demand_cl_poblation_o, inventario, costo_instalaciones_cl, costo_instalaciones_cr, costo_compraproductos, costo_transporte, costo_inventario, costo_rutas_s, costo_rutas_p, costo_vehiculos_s, costo_vehiculos_p, costo_humano, n_generaciones, fitness_dict_o, demand_cr_poblation_o, Q_poblation_o, I_poblation_o, f1_poblation_o, f2_poblation_o, prob_mut, colector, idx_pa, timer):

    for _ in tqdm(range(1, n_generaciones+1)):
        n_poblacion = len(p_externa)
        # inicio operadores geneticos
        # seleccion de los padres
        idx_parents = selection_padres(len(p_externa), fitness_dict_o)
        # cruce
        p_crossed, hijos, demand_cr_hijos, demand_cl_hijos, Q_hijos, I_hijos, f1_hijos, f2_hijos = crossover(p_externa, idx_parents, n_clientes, n_centroslocales, n_centrosregionales, n_periodos, n_productos, n_vehiculos_s, n_vehiculos_p, capacidad_cr, capacidad_cl, capacidad_vehiculos_p, capacidad_vehiculos_s, demanda_clientes, demand_cl_poblation_o, inventario, costo_instalaciones_cl, costo_instalaciones_cr, costo_compraproductos, costo_transporte, costo_inventario, costo_rutas_s, costo_rutas_p, costo_vehiculos_s, costo_vehiculos_p, costo_humano)
        # mutacion
        ...
        demand_cr_poblation_f = []
        demand_cl_poblation_f = []
        Q_poblation_f = []
        I_poblation_f = []
        f1_poblation_f = []
        f2_poblation_f = []
        for padre in idx_parents:
            demand_cr_poblation_f.append(demand_cr_poblation_o[padre])
            demand_cl_poblation_f.append(demand_cl_poblation_o[padre])
            Q_poblation_f.append(Q_poblation_o[padre])
            I_poblation_f.append(I_poblation_o[padre])
            f1_poblation_f.append(f1_poblation_o[padre])
            f2_poblation_f.append(f2_poblation_o[padre])
        # consolidacion de la nueva poblacion
        big_poblation = p_crossed + hijos
        demand_cr_big_poblation = demand_cr_poblation_f + demandas_cr_hijos
        demand_cl_big_poblation = demand_cl_poblation_f + demand_cl_hijos
        Q_big_poblation = Q_poblation_f + Q_hijos
        I_big_poblation = I_poblation_f + I_hijos
        f1_big_poblation = f1_poblation_f + f1_hijos
        f2_big_poblation = f2_poblation_f + f2_hijos
            ...
            while len(ind_k) > n_poblacion:
                for idd, den in densidades_dict.items():
                    if idd in idx_externa:
                        no_den = np.where(np.array(idx_externa) == idd)
                        ind_k.pop(no_den[0][0])
                        idx_externa.pop(no_den[0][0])
                        if len(ind_k) < n_poblacion:
                            ""
                        else:
                            break
        ...
        for f, idg in enumerate(idx_externa):
            f1_poblation_o.append(f1_big_poblation[idg])
            f2_poblation_o.append(f2_big_poblation[idg])
            fitness_dict_o[f] = fitness_dict[idg]
            demand_cr_poblation_o.append(demand_cr_big_poblation[idg])
            demand_cl_poblation_o.append(demand_cl_big_poblation[idg])
            Q_poblation_o.append(Q_big_poblation[idg])
            I_poblation_o.append(I_big_poblation[idg])
        ...
        Q_temp = []
        I_temp = []
        for fit_i, fit_o in fitness_dict_o.items():
            if fit_o[0] <= 1:
                id_colec.append(fit_i)
            else:
                ""
        for id_c in id_colec:
            ind_temp.append(p_externa[id_c])
            f1_temp.append(f1_poblation_o[id_c])
            f2_temp.append(f2_poblation_o[id_c])
            fit_temp[id_c] = fitness_dict_o[id_c]
            dem_cr_temp.append(demand_cr_poblation_o[id_c])
            dem_cl_temp.append(demand_cl_poblation_o[id_c])
            Q_temp.append(Q_poblation_o[id_c])
            I_temp.append(I_poblation_o[id_c])
        colector.value[idx_pa] = ind_temp, dem_cr_temp, dem_cl_temp, Q_temp, I_temp, f1_temp, f2_temp, fit_temp,
        time.sleep(timer)
        ...
        replaced = []
        transfer = []
        while len(replaced) < int(0.1*n_poblacion):
            id_r = np.random.randint(len(p_externa))
            id_a = np.random.randint(len(colector.value[idx_pa][0]))
            while id_r in replaced and id_a in transfer:
                id_r = np.random.randint(len(p_externa))
                id_a = np.random.randint(len(colector.value[idx_pa][0]))
            p_externa[id_r] = colector.value[idx_pa][0][id_a]
            demand_cr_poblation_o[id_r] = colector.value[idx_pa][1][id_a]
            demand_cl_poblation_o[id_r] = colector.value[idx_pa][2][id_a]
            Q_poblation_o[id_r] = colector.value[idx_pa][3][id_a]
            I_poblation_o[id_r] = colector.value[idx_pa][4][id_a]
            f1_poblation_o[id_r] = colector.value[idx_pa][5][id_a]
            f2_poblation_o[id_r] = colector.value[idx_pa][6][id_a]
            replaced.append(id_r)
            transfer.append(id_a)

    # almacenaje de la mejor aproximacion de pareto para cada isla
    pareto = [p_externa, demand_cr_poblation_o, demand_cl_poblation_o, Q_poblation_o, I_poblation_o, f1_poblation_o, f2_poblation_o]
    colector.value[2].append(pareto)

    return p_externa, demand_cr_poblation_o, demand_cl_poblation_o, Q_poblation_o, I_poblation_o, f1_poblation_o, f2_poblation_o
