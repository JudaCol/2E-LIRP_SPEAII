...
def adaptability(n_poblacion, valores_f1, valores_f2):
    # construccion de frentes de la poblacion inicial
    dominancias = {}  # diccionario individuo:dominancias
    ind_domina = {}  # diccionario individio: inidividuos que lo dominan
        ...
        for k in range(n_poblacion):
            if j == k:
                ...
                if ((valores_f1[j] >= valores_f1[k]) and (valores_f2[j] >= valores_f2[k])) and ((valores_f1[j] > valores_f1[k]) or (valores_f2[j] > valores_f2[k])):
                    n += 1
                    ind_domina[j].append(k)
                else:
                    # print("a ver a ver que paso acá")
                    continue
        dominancias[j] = n

    return dominancias, ind_domina


def fuerzas_fun(n_poblacion, valores_f1, valores_f2):
    # construccion de frentes de la poblacion inicial
    fuerzas = {}  # diccionario individuo:dominancias
    for j in range(n_poblacion):
        n = 0
        for k in range(n_poblacion):
            if j == k:
...
def fitness(n_poblacion, valores_f1, valores_f2):
    # calculo de fuerza de los individuos
    fuerzas = fuerzas_fun(n_poblacion, valores_f1, valores_f2)
    # calculo de adaptabilidad bruta
    dominancias, ind_domina = adaptability(n_poblacion, valores_f1, valores_f2)
    adaptability_dict = {}
    for ind, ind_d in ind_domina.items():
        adaptability_dict[ind] = 0
        for ind_ds in ind_d:
            adaptability_dict[ind] += fuerzas[ind_ds]
    # calculo de la matriz de densidad
    densidades = np.zeros((n_poblacion, n_poblacion))
    for h in range(n_poblacion):
        for v in range(n_poblacion):
            if h == v:
                densidades[h][v] = 999999999999999999999999
            else:
                densidades[h][v] = np.sqrt(((valores_f1[h] - valores_f1[v]) ** 2) + ((valores_f2[h] - valores_f2[v]) ** 2))
    densidades_f = {}
    for idx, densidad in enumerate(densidades):
        densidades_f[idx] = 1 / (densidad.min() + 2)
    # calculo final del fitness
    fitness_f = {}
    for idf in range(n_poblacion):
        fitness_f[idf] = adaptability_dict[idf] + densidades_f[idf]

    return fitness_f, densidades_f


def fit_den_pext(n_poblacion, valores_f1, valores_f2, individuos, demand_cr_poblation, demand_cl_poblation, Q_poblation, I_poblation):
    idx_externa = []
    p_externa = []
    ind_k = []
    ...
    for f, idg in enumerate(idx_externa):
        f1_poblation_o.append(valores_f1[idg])
        f2_poblation_o.append(valores_f2[idg])
        fitness_dict_o[f] = fitness_dict[idg]
        demand_cr_poblation_o.append(demand_cr_poblation[idg])
        demand_cl_poblation_o.append(demand_cl_poblation[idg])
        Q_poblation_o.append(Q_poblation[idg])
        I_poblation_o.append(I_poblation[idg])
    return p_externa, f1_poblation_o, f2_poblation_o, fitness_dict_o, demand_cr_poblation_o, demand_cl_poblation_o, Q_poblation_o, I_poblation_o
